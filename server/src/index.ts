...
const Main = async () => {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("send", async (message: string, room) => {
      let sqlSafe = message.replace(/'/g, "/'")
      sqlSafe = sqlSafe.replace(/;/g, "/;")
      try {
        await Messages.query(
          `INSERT INTO public.messages(username,content,room) VALUES ('${socket.handshake.query.username}','${sqlSafe}','${room}')`
        )
        socket
          .to(room)
